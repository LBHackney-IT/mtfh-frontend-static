...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/tl-housing-static-frontend/state"
  }
}
resource "aws_s3_bucket" "frontend-bucket-development" {
  bucket = "lbh-tl-housing-static-frontend-development.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
...
module "cloudfront-development" {
  ...
  origin_id = "mtfh-tl-housing-static-frontend"
  s3_bucket_arn = aws_s3_bucket.frontend-bucket-development.arn
  s3_bucket_id = aws_s3_bucket.frontend-bucket-development.id
  orginin_access_identity_desc = "tl-housing static frontend cloudfront identity"
  cname_aliases = []
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/tl-housing/development/static-app-url"
  type  = "String"
